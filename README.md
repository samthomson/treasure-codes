...
Batch entrypoint via a text file: **one URL per line** (lines starting with `#` are comments).
Defaults to QR plates and outputs `qr_01.3mf`, `qr_02.3mf`, … into `output/` unless you pass `-d`.
...
python generate_all.py urls.txt --mode container --container-size medium
python generate_all.py urls.txt --mode container --container-size small
python generate_all.py urls.txt --mode container --container-size large
python generate_all.py urls.txt --mode container_lid -v large
python generate_all.py urls.txt --mode container_full -v large --container-size medium
```
...
python generate_container.py "https://treasures.to/naddr1qfjunkbbbqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqujunkbbb" --base-only --container-size small -o ./out/box_small.3mf
python generate_container.py "https://treasures.to/naddr1qfjunkbbbqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqujunkbbb" --base-only --container-size large -o ./out/box_large.3mf
python generate_container.py "https://treasures.to/naddr1qfjunkbbbqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqujunkbbb" --lids-only -o ./out/lid.3mf
```

**`-v`**: `large` (default) · `small`.  
**`--container-size`**: `small` (0.5x) · `medium` (default) · `large` (2x), bottom only.  
**`--base-only`**: export only container bottom (omit lid + QR).  
**`-s`**: override QR size in mm.  
**`--lids-only`**: export only lid + QR (omit container base).  
